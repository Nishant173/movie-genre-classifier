...
def get_train_test_data():
    ...
    if config.LOAD_PREPROCESSED_DATA:
        try:
            ...
            raise Exception("Pickle of preprocessed data not found. Please set config.LOAD_PREPROCESSED_DATA to False (for now), and redo data preprocessing")
    else:
...
def train_model(df_movie_data):
    """Trains model and returns dictionary containing info about said model"""
...
def evaluate_model(ovr_clf, dictionary_train_info):
    """
    Evaluates the model based on it's classifier object and some training info.
